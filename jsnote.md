...
29. 重写原型之后，在重写原型之前new的对象依旧指向旧的原型，在重写原型之后new的实例指向新的原型。
...
- 父级模板里的所有内容都是在父级作用域中编译的；子模板里的所有内容都是在子作用域中编译的。(插槽作用域问题)
...
## 模块化
1. 模块基本要求：隐藏实现细节、定义模块接口（通过接口可以调用模块的功能）。
2. AMD：基于浏览器，自动处理依赖，无需考虑模块引入的顺序；异步加载模块，避免阻塞；在同一个文件中可以定义多个模块
3. CommonJS：面向通用js环境，基于文件的模块，文件同步加载可以访问模块公共接口，加载更快，只需要读取文件系统，服务端更常用，语法简单，只需定义module.exporsts属性，只需要使用require函数
4. ES6模块结合了commonjs和amd优点，基于文件也支持异步加载模块，显示导出模块 export 和 import

## 事件循环
- 宏任务：创建主文档对象、解析html、执行全局js代码、更改当前url、页面加载、输入、网络事件和定时器事件，从浏览器角度来看，宏任务代表一个个离散的、独立的工作单元。运行完后浏览器继续其他调度，重新渲染页面ui或执行垃圾回收
- 微任务：更新应用程序的状态，但必须在浏览器任务继续执行其他任务之前执行，浏览器任务包括重新渲染也买你ui，微任务包括promise回调函数、dom发生变化等。使得我们能在重新渲染UI之前执行指定的行为，避免不必要的UI重绘。
- 事件循环遵循两个原则： 一次处理一个任务、一个任务开始后直到运行完成，不会被其他任务中断。
- 单次循环迭代中，最多处理一个宏任务，而队列中的所有微任务都会被处理。js基于单线程执行模型，这两类任务都是逐个执行的。所有微任务会在下一次渲染之前执行完成，因为他们目标是在渲染前更新应用程序状态。浏览器会尝试60fps速度渲染页面。
- 宏任务包括：鼠标事件，键盘事件，网路事件，html解析
- 微任务包括：DOM变化、Promises